...
class Admin {
	...
	function assets() {
		...
		if ( 'tools_page_geo-ip-library-settings' == $current_screen->id ) {
	...
	function l10n_assets() {
		$l10n_geo_ip = array(
			...
			'UPDATE_AVAILABLE_IN' => __( 'UPDATE AVAILABLE IN %1$d %2$s', 'geo-ip-library' ),
	 ...
	 */
	function settings() {
		require( GEO_IP_LIBRARY_PATH . 'includes/class-htmlizer.php' );
		...
		$vars = array(
			...
			'GET_CLIENT_COUNTRY_CODE'        => sprintf( __( 'Returns the current client\'s %1$s country code or the specified at %2$s parameter.', 'geo-ip-library' ), '<a href="https://wikipedia.org/wiki/ISO_3166-1_alpha-2">ISO 3166-1 alpha-2</a>', '<span style="color: #369">$ip</span>' ),
